...
namespace Game
{
	...
	void init()
	{
		...
		for (int i = 0; i < n_bricks; i++)
		{
			...
			for (int j = 3; j < n_bricks; j++)
			{

				brick_state[i] = 1;
				brick_pos[j].x = x % screen_width;
				// set x+= to 100, 70, or 122, to see different ways to sort the blocks. 100 is the in collumns and rows default.
				x += 100;
				brick_pos[j].y = y % screen_height;
				y += 2;
			}
	...
	void update()
	{
		...
		int result = Collision::minkowski(
			ball_pos.x, ball_pos.y, ball_size.w, ball_size.h,
			padel_pos.x, padel_pos.y, padel_size.w, padel_size.h);
	...
	{
		SDL_SetRenderDrawColor(renderer, ball_color.r, ball_color.g, ball_color.b, 255);
		SDL_Rect rect = { padel_pos.x,padel_pos.y,padel_size.w,padel_size.h };
		SDL_RenderFillRect(renderer, &rect);
	}
	void draw_Fancy_Padel()
	{
		SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
		SDL_Rect rect0 = { padel_pos.x,padel_pos.y,padel_size.w,padel_size.h };
		SDL_RenderFillRect(renderer, &rect0);

		SDL_SetRenderDrawColor(renderer, padel_color.r, padel_color.g, padel_color.b, 255);
		SDL_Rect rect1 = { padel_pos.x + 2,padel_pos.y + 2,padel_size.w - 4,padel_size.h - 4 };
		SDL_RenderFillRect(renderer, &rect1);
	}
}
